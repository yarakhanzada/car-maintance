...
class ProfileScreen extends StatelessWidget {
    ...
    return Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: const Color(0xFFE55757), width: 2)),
          child: const CircleAvatar(radius: 32, backgroundColor: Color(0xFFE0E0E0), child: Icon(Icons.person, size: 32, color: Colors.white)),
        ),
        const SizedBox(width: 15),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(user.name ?? "", style: const TextStyle(fontSize: 19, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
            Text(user.email ?? "", style: const TextStyle(color: Colors.grey, fontSize: 12.5)),
          ],
        ),
      ],
    );
  ...
  Widget _buildSubscriptionCard(BuildContext context, double width) {
    return Obx(() {
      if (subscriptionController.isLoading.value && subscriptionController.mySubscriptions.isEmpty) return const Center(child: CircularProgressIndicator());
      ...
      if (rawDate.isNotEmpty) {
        try { DateTime dateTime = DateTime.parse(rawDate); formattedDate = DateFormat('yyyy-MM-dd').format(dateTime); } catch (e) { formattedDate = "تاريخ غير صالح"; }
      }
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: cardColors[0].withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 8))],
        ),
        child: Stack(
          children: [
            Positioned(
              left: 20,
              bottom: -15,
              child: Icon(Icons.workspace_premium, size: 110, color: Colors.white.withOpacity(0.1)),
            ),
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(planName.toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 14)),
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: _buildInfoItem(Icons.calendar_today, "ينتهي في: $formattedDate")),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildLogoutTile() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(
        children: [
          Container(width: 42, height: 42, decoration: BoxDecoration(color: const Color(0xFFF8F9FA), borderRadius: BorderRadius.circular(12)), child: const Center(child: LogoutWidget())),
          const SizedBox(width: 15),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text("تسجيل الخروج", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF1A1A1A))),
                Text("اخرج من حسابك", style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildMenuTile({required IconData icon, required String title, required String subtitle, required VoidCallback onTap}) {
    return Container(
      ...
      child: ListTile(
        ...
        trailing: const Icon(Icons.arrow_back_ios_new_rounded, size: 12, color: Colors.black12),
      ),
    );
  }
  ...
  Widget _buildEmptySubscription(double width) {
    return Container(
      ...
      child: Row(
        children: [
          Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(color: Color(0xFFFEEAEA), shape: BoxShape.circle), child: const Icon(Icons.account_balance_wallet_rounded, color: Color(0xFFE55757), size: 24)),
          const SizedBox(width: 15),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text("لا توجد باقة نشطة", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: Color(0xFF1A1D26))),
                const SizedBox(height: 4),
                Text("اشترك في إحدى باقاتنا المتاحة لفتح الميزات المميزة", style: TextStyle(color: const Color(0xFF2D3243).withOpacity(0.5), fontSize: 12, height: 1.4)),
              ],
            ),
          ),
        ],
